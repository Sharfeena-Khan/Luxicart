...
const getAddProduct = async(req, res)=>{
    try {
        ...
        res.render("add-Product", { title:`Luxicart-Add Products`, categories, error: req.flash('error')})
...
const insertProduct = async(req, res) =>{
    ...
    const categories = await Category.find();
    const { name,  discription, price, discountType, percentage, SKU, qty, category, status } = req.body
    ...
    const insertProductValidationRules = [
        ...
        body('price').isNumeric().withMessage('Price must be a number').matches(/^\d*\.?\d+$/),
    ...
    try {
        await Promise.all(insertProductValidationRules.map(validation => validation.run(req)));
        ...
        if (!error.isEmpty()) {
            ...
            req.flash('error', error.array().map(error => error.msg));
            // req.flash('error', 'Please provide valid values for all required fields.');
            return res.render('add-Product', { title: 'Luxicart-Products',error: req.flash('error'),categories });
        }
...
const categories = await Category.find()
    try {
        ...
        res.render("editProduct" , { title:`Luxicart- Edit Products`, products, categories,  error: req.flash('error')  })
    } catch (error) {
        console.log(error);
    }
}
...
const updateProduct = async (req, res) => {
    ...
    try {
        ...
        const { name, image, description, price, discountType,
             percentage, SKU, Qty, category, status , ID } = req.body;
        ...
        const insertProductValidationRules = [
            ...
            body('percentage').isNumeric().withMessage('Percentage must be a number'),
            body('SKU').notEmpty().withMessage('SKU is required'),
             body('Qty').isNumeric().withMessage('Quantity must be a number').matches(/^\d*\.?\d+$/),
            body('category').notEmpty(),
            body('status').notEmpty().withMessage('Status is required'),
        ];
        ...
        if (!errors.isEmpty()) {
            ...
            return res.render("editProduct", { title: `Luxicart- Edit Products`, products, categories, error: req.flash('error') });
        }
        if(products){
           products.name= name 
...
function isValidImage(file) {
    ...
    if (!allowedExtensions.exec(file.originalname) || file.size > maxSizeInBytes) {
        return false;
    }
